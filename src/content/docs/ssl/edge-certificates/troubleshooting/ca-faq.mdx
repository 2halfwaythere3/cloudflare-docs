...
`An error occured while attempting to validate your domain. Please try aagin later of contact support for assistance.`

Please check the status of the certificate in the [Cloudflare Dashboard](https://dash.cloudflare.com?to=/:account/:zone/ssl-tls): if the status is `Active`, you can disregard this email and the error message.
